...
/**
 * KubeVirt VM 상세(OpenShift Virtualization VM 상세 등가) — 탭형(개요·YAML·이벤트·콘솔).
 * VM + VMI + virt-launcher 파드를 조인해 세부정보·일반·네트워크·스토리지를 보이고,
 * Start/Stop/Restart/Pause 라이프사이클 액션을 제공. 콘솔 탭은 별도 컴포넌트(stage 2/3)로 채운다.
 * 콘솔 그룹 임퍼소네이션 write로 라이프사이클 수행.
 */
@Component({
  ...
  styles: [`
    ...
    .vm-kv { display: grid; grid-template-columns: 130px 1fr; gap: .35rem .75rem; padding: .25rem 0; }
      ...
      <div>
        ...
        <div class="card">
          ...
          <clr-datagrid>
            ...
            <clr-dg-row *clrDgItems="let n of networks()">
              <clr-dg-cell>{{ n.name }}</clr-dg-cell><clr-dg-cell>{{ n.ip || '—' }}</clr-dg-cell>
            </clr-dg-row>
      ...
      <div>
        ...
        <div class="card">
          ...
          <clr-datagrid>
            ...
            <clr-dg-row *clrDgItems="let d of disks()">
              <clr-dg-cell>{{ d.name }}</clr-dg-cell><clr-dg-cell>{{ d.drive }}</clr-dg-cell><clr-dg-cell>{{ d.bus }}</clr-dg-cell><clr-dg-cell>{{ d.source }}</clr-dg-cell>
            </clr-dg-row>
...
export class VmDetailComponent implements OnInit {
  ...
  load(): void {
    ...
    forkJoin({
      ...
      pods: this.k8s.list('/api/v1/namespaces/' + this.namespace + '/pods').pipe(catchError(() => of({ items: [] as any[] }))),
  ...
  setRunning(on: boolean) {
    ...
    this.k8s.patchMerge(this.vmPath(), patch).subscribe({
      next: () => { this.busy.set(false); this.flash(on ? 'VM 시작 요청.' : 'VM 정지 요청.', true); this.changed.emit(); setTimeout(() => this.load(), 600); },
  ...
  restart() {
    ...
    this.k8s.post(this.subPath('restart'), {}).subscribe({
      next: () => { this.busy.set(false); this.flash('VM 재시작 요청.', true); this.changed.emit(); setTimeout(() => this.load(), 600); },
  ...
  pause(on: boolean) {
    ...
    this.k8s.post(this.subPath(on ? 'pause' : 'unpause'), {}).subscribe({
      next: () => { this.busy.set(false); this.flash(on ? 'VM 일시정지.' : 'VM 재개.', true); this.changed.emit(); setTimeout(() => this.load(), 600); },
