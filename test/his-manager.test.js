...
test('HIS catalog keeps PFS/plugin concepts outside the prerequisite catalog', () => {
  ...
  assert.match(crossplaneValues.provider.packages[0], /^xpkg\.crossplane\.io\/.+@sha256:[a-f0-9]{64}$/);
  assert.equal(crossplaneKindValues.image.ignoreTag, true);
  assert.equal(crossplaneKindValues.image.repository, crossplane.runtimePolicy.kind.coreImage);
  assert.deepEqual(crossplaneKindValues.provider.packages, []);
  const deploymentRuntimeConfig = crossplaneKindValues.extraObjects.find((item) => item.kind === 'DeploymentRuntimeConfig');
  const provider = crossplaneKindValues.extraObjects.find((item) => item.kind === 'Provider');
  assert.equal(deploymentRuntimeConfig.metadata.name, crossplane.runtimePolicy.kind.runtimeConfigName);
  assert.equal(deploymentRuntimeConfig.spec.deploymentTemplate.spec.template.spec.containers[0].image, crossplane.runtimePolicy.kind.providerRuntimeImage);
  assert.equal(provider.spec.package, crossplane.runtimePolicy.kind.providerPackage);
  assert.equal(provider.spec.runtimeConfigRef.name, deploymentRuntimeConfig.metadata.name);
  const packageMirror = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../his-mirrors/provider-helm-package.json'), 'utf8'));
  assert.equal(`ghcr.io/opensphere-platform/mirror-crossplane-provider-helm-package@${packageMirror.resolvedDigest}`, provider.spec.package);
  ...
  const argoCd = catalogItem('argocd-runtime');
  assert.equal(argoCd.mode, 'HelmManaged');
  assert.equal(argoCd.required, false);
  ...
  assert.ok(argoCd.replacementPolicy.allowedKinds.includes('Deployment'));
  assert.ok(!argoCd.replacementPolicy.allowedKinds.includes('CustomResourceDefinition'));
  assert.ok(argoCd.replacementPolicy.preserved.some((entry) => /Application\/AppProject/.test(entry)));
  assert.match(argoCd.values.join(' '), /global\.image\.tag=v3\.4\.2@sha256:c612d570/);
  assert.ok(argoCd.retainedOnDelete.some((entry) => /Application\/AppProject/.test(entry)));
  assert.match(dockerfile, /argo-cd --repo https:\/\/argoproj\.github\.io\/argo-helm --version 9\.5\.15/);
  assert.match(dockerfile, /95502bea856e2e1e9bfbb7a5ab90d309970b4ffd098ff3dadb99188350768b9e/);
  const managerSource = fs.readFileSync(path.resolve(__dirname, '../his-manager.js'), 'utf8');
  assert.match(managerSource, /prepareManagedReplacement\(ctx, actor, item, current, variant, managedValues\)/);
  assert.match(managerSource, /takeOwnership\s*\?\s*\['--take-ownership'\]/);
  assert.match(managerSource, /operation\.replacementApproved/);
...
test('HIS status contract uses the constitution-defined acronym and versioned schema', () => {
  assert.equal(HIS_STATUS_SCHEMA, 'his-status.opensphere.io/v1alpha1');
...
test('signed HIS owner release includes bounded lifecycle RBAC for fixed namespaces', () => {
  ...
  assert.deepEqual(bindings.map((binding) => binding.metadata.namespace).sort(), ['argocd', 'cert-manager', 'crossplane-system', 'ingress-nginx', 'kube-system', 'monitoring', 'opensphere-console']);
